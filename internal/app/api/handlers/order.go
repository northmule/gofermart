...
func (o *OrderHandler) UploadingOrder(res http.ResponseWriter, req *http.Request) {
	...
	// Создаю баланс
	err = o.accrualService.CreateJobToProcessNewOrder(req.Context(), newOrder.Number, user.UUID)
	if err != nil {
		logger.LogSugar.Errorf(err.Error())
		res.WriteHeader(http.StatusInternalServerError)
		return
	}
	// Задание на обработку заказа
	err = o.jobService.CreateJobToProcessNewOrder(req.Context(), newOrder.Number)
	if err != nil {
		logger.LogSugar.Errorf(err.Error())
		res.WriteHeader(http.StatusInternalServerError)
		return
